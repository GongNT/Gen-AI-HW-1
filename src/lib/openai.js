...
export async function evaluateVisualReactions(metadata, frames) {
  const content = [
    {
      type: 'text',
      text: `You are analyzing webcam snapshots of a person watching a YouTube video titled "${metadata.title}" (duration ${metadata.durationSeconds}s). The frames below are sampled in two phases: first, up to 10 frames spaced evenly across the first 30 seconds of viewing; then (if the video is longer and they kept watching), up to 10 additional frames from later in the video, each chosen because it showed the biggest moment-to-moment change in the viewer's expression. Each frame is labeled with its approximate timestamp. Describe the viewer's visible facial expressions and reactions over time, calling out specific moments (with timestamps) where their expression noticeably changed (e.g., smiling, laughing, frowning, surprised, bored, neutral) — the later frames in particular mark the viewer's strongest reaction shifts. Write a clear, structured evaluation organized by timestamp.`,
    },
  ]
