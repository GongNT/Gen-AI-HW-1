...
export async function evaluateVisualReactions(metadata, frames) {
  const content = [
    {
      type: 'text',
      text: `You are analyzing webcam snapshots of a person watching a YouTube video titled "${metadata.title}" (duration ${metadata.durationSeconds}s). Below are up to 20 frames captured at roughly even intervals across the viewing, each labeled with its approximate timestamp in the video. Describe the viewer's visible facial expressions and reactions over time, calling out specific moments (with timestamps) where their expression noticeably changed (e.g., smiling, laughing, frowning, surprised, bored, neutral). Write a clear, structured evaluation organized by timestamp.`,
    },
  ]
